(function (prosemirrorState, prosemirrorView, prosemirrorSchemaBasic, prosemirrorExampleSetup, prosemirrorTransform) {
  ...
  const trackPlugin = new prosemirrorState.Plugin({
    state: {
      init(_, instance) {
  ...
  const highlightPlugin = new prosemirrorState.Plugin({
    state: {
      init() { return {deco: prosemirrorView.DecorationSet.empty, commit: null} },
      apply(tr, prev, oldState, state) {
        ...
        if (highlight && highlight.add != null && prev.commit != highlight.add) {
          ...
          let decos = tState.blameMap
              ...
              .map(span => prosemirrorView.Decoration.inline(span.from, span.to, {class: "blame-marker"}));
  ...
  view = window.view = new prosemirrorView.EditorView(document.querySelector("#editor"), {state, dispatchTransaction: dispatch});
  ...
  function revertCommit(commit) {
    ...
    let remap = new prosemirrorTransform.Mapping(trackState.commits.slice(index)
